...
export class ProjectsService {
  ...
  async findAllByUser(userId: string, role: string) {
    ...
    return this.prisma.project.findMany({
      where: {
        OR: [{ userId }, { stakeholders: { some: { profileId: userId } } }],
      },
      include: includeRelations,
    });
  }
  ...
  async findOne(id: string, userId: string, role: string) {
    ...
    if (role !== 'admin') {
      if (role === 'developer') {
        ...
        const isInvitedStakeholder = project.stakeholders.some(
          (s) => s.profileId === userId,
        );
    ...
    const totalChecklists = project.tasks.reduce(
      (acc, task) => acc + task.checklists.length,
      0,
    );
    const checkedChecklists = project.tasks.reduce(
      (acc, task) => acc + task.checklists.filter((c) => c.isChecked).length,
      0,
    );
    const progress =
      totalChecklists === 0
  ...
  async findBySlug(slug: string) {
    const project = await this.prisma.project.findUnique({
      ...
      include: {
        tasks: {
          include: { checklists: true, screenshots: true },
  ...
  async removeStakeholder(projectId: string, profileId: string) {
    ...
    if (!record)
      throw new NotFoundException('Stakeholder assignment not found');
